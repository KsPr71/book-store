    ...
    <div onClick={handleCardClick} className="cursor-pointer w-full">
      <CardContainer className="inter-var w-full" containerClassName="py-0 w-full">
        <CardBody className="bg-gray-50 relative group/card dark:hover:shadow-2xl dark:hover:shadow-blue-500/[0.1] dark:bg-gradient-to-br dark:from-neutral-900 dark:via-neutral-800 dark:to-neutral-900 dark:border-blue-500 border-black/[0.1] w-full h-auto rounded-xl p-3 border transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/30 dark:hover:shadow-blue-500/50 hover:border-blue-500/60 dark:hover:border-blue-400 hover:bg-gray-100 dark:hover:from-neutral-800 dark:hover:via-neutral-700 dark:hover:to-neutral-800">
              ...
              <div className="relative w-full aspect-[2/3] rounded-lg overflow-hidden shadow-md group-hover/card:shadow-xl transition-all duration-300">
                ...
                {firstCategory && (
                  <div className="absolute bottom-2 left-2 z-10 opacity-0 group-hover/card:opacity-100 transition-all duration-300 transform translate-y-2 group-hover/card:translate-y-0">
                    <CardItem
                      translateZ={25}
                      className="inline-block px-2 py-0.5 rounded-lg text-[10px] font-semibold bg-blue-500/80 dark:bg-blue-600/80 text-white backdrop-blur-sm shadow-lg group-hover/card:scale-110 transition-transform duration-300"
                    >
          ...
          {/* Precio y Botón de WhatsApp - Compartiendo línea */}
          <div 
          ...
          >
            {/* Precio - Parte inferior izquierda */}
            <CardItem
              translateZ={30}
              className="px-2.5 py-1.5 rounded-lg bg-neutral-200/80 dark:bg-neutral-700/80 backdrop-blur-sm text-neutral-700 dark:text-neutral-200 font-bold shadow-sm transition-all duration-300 group-hover/card:bg-blue-500 group-hover/card:text-white dark:group-hover/card:bg-blue-600 dark:group-hover/card:text-white"
              style={{
                fontSize: `${Math.max(0.7, cardSize * 0.04)}rem`
              }}
            >
            ...
            <a
              ...
              onMouseDown={(e) => e.stopPropagation()}
              className="flex items-center justify-center rounded-xl bg-[#25D366] hover:bg-[#20BA5A] text-white transition-colors duration-200 cursor-pointer flex-shrink-0"
              style={{
                padding: `${Math.max(0.5, cardSize * 0.022)}rem ${Math.max(0.75, cardSize * 0.035)}rem`,
                width: `${Math.max(2.75, cardSize * 0.135)}rem`,
                height: `${Math.max(2.25, cardSize * 0.11)}rem`
              }}
            >
              <svg
                ...
                fill="currentColor"
                style={{
                  width: `${Math.max(1.125, cardSize * 0.055)}rem`,
                  height: `${Math.max(1.125, cardSize * 0.055)}rem`
                }}
              >
